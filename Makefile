...
src/git2.nci:
	...
	echo -e "[package]\nGit2::Raw\n\n[lib]\n\nlibgit2.so\n\n[defs]\n" >> src/git2.nci ; \
	perl ./bin/header2nci.pl `pkg-config --variable=includedir libgit2`/git2/git2.h >> src/git2.nci ; \
	for i in `pkg-config --variable=includedir libgit2`/git2/*; do  perl ./bin/header2nci.pl $$i >> src/git2.nci ; done
